...
const loadFromStorage = (key, defaultValue) => {
  try {
    ...
    // Special handling for chat messages with timestamps
...
export default function App() {
  ...
  // Estados independientes por modo con persistencia
  ...
  // Determinar si hasRun basado en si hay datos existentes
  ...
  const resetAllData = useCallback(() => {
    ...
    if (confirmed) {
      // Limpiar solo los resultados generados, mantener fuentes
      setSummaryData("");
      setQuizData([]);
      setFlashcardsData([]);
      setChatMessages([]);
      ...
      // Limpiar solo los datos generados de localStorage, mantener fuentes
  ...
  const combinedText = useMemo(
    () => buildStudyMaterialText(text, uploads),
    [text, uploads]
  );
    ...
    async (files) => {
      ...
      try {
        ...
        for (const file of files) {
          ...
          if (!trimmed) {
            throw new Error(
              `“${file.name}” has no extractable text. Try another file or paste the text instead.`
            );
          }
          next.push({
  ...
  const run = useCallback(async () => {
    ...
    // Para chat, agregar mensaje del usuario inmediatamente
    if (mode === "chat") {
    ...
    try {
      ...
      // Setear el estado correcto según el modo (solo después de respuesta exitosa)
      if (mode === "summary") {
        setSummaryData(out);
      } else if (mode === "quiz") {
        setQuizData(out);
        ...
        // Para chat, agregar respuesta del asistente
        setChatMessages((prev) => [
          ...prev,
          {
            role: "assistant",
